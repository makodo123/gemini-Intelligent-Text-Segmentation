 ...
 * Splits segments based on punctuation (。！？?!) and interpolates timestamps.
 */
export const smartSplitSegments = (segments: TranscriptSegment[]): TranscriptSegment[] => {
  ...
  for (let i = 0; i < segments.length; i++) {
    ...
    // Skip splitting if text is too short or doesn't contain sentence terminators
    if (text.length < 10 || !/[。！？?!]/.test(text)) {
      newSegments.push(segment);
      continue;
    }

    // Determine the duration of the current segment
    const nextSegment = segments[i + 1];
    let duration = 0;
    
    if (nextSegment) {
      duration = nextSegment.startTimeSeconds - segment.startTimeSeconds;
      // Sanity check: if gap is huge (e.g. > 30s) or negative, assume a default reading speed
      if (duration <= 0 || duration > 60) {
        duration = text.length * 0.3; // Approx 0.3s per char
      }
    } else {
      // Last segment
    ...
    // Split text by punctuation, keeping the punctuation attached to the previous part
    // Regex lookbehind: Split after these characters
    ...
    parts.forEach((part) => {
      // Calculate how much time this part takes relative to total length
      // We add a tiny buffer to avoid identical timestamps
